...
# 2. Выведите столбцы id, order_id из таблицы order_details, а также вычисляемый столбец category в
# зависимости от значений unit_price.
...
# 3. Вывести все строки там, где purchase_order_id не указано. При этом дополнительно создать столбец
# total_price как произведение quantity * unit_price.
SELECT 
...
# 5. На основе таблицы orders вывести один столбец - с годом и месяцем из order_date в формате 'годмесяц'.
SELECT 
    SUBSTR(order_date, 1, 7) AS YYYYMM
FROM
    orders;
